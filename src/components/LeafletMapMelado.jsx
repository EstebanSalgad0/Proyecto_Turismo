...
const LeafletMap = () => {
  ...
  const rioMeladoCoords = [-35.847518970858225, -71.05518511993768];
  let userMarker;
  let userCoords;
  let routeControl;
  let transport = 'car';
  ...
  useEffect(() => {
    if (!mapRef.current) {
      const map = L.map('map', { zoomControl: false }).setView(rioMeladoCoords, 13);
      mapRef.current = map;
      ...
      let meladoMarker = L.marker(rioMeladoCoords).addTo(map).bindPopup("El Melado", { closeOnClick: false });
      ...
      function updateRoute() {
        ...
        routeControl = L.Routing.control({
          waypoints: [L.latLng(userCoords), L.latLng(rioMeladoCoords)],
      ...
      function togglePopups() {
        if (meladoMarker.isPopupOpen()) {
          meladoMarker.closePopup();
          setTimeout(() => userMarker.openPopup(), 200);
        } else {
          userMarker.closePopup();
          setTimeout(() => meladoMarker.openPopup(), 200);
        }
      ...
      function openInGoogleMaps() {
        if (userCoords) {
          ...
          const meladoLatLng = `${rioMeladoCoords[0]},${rioMeladoCoords[1]}`;
          const googleMapsUrl = `https://www.google.com/maps/dir/${userLatLng}/${meladoLatLng}/`;
